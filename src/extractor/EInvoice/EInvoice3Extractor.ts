...
export class EInvoice3Extractor extends EInvoice2Extractor {
  ...
  protected override processPage(pageLines: string[]) {
    try {
      ...
      let rowRegex =
        /^\d+\#[A-ZÁÀẠÃẢẮẰẲẶẴẤẦẬẨẪĐÓÒỎỌÕÔỐỒỔỘỖƠỚỜỞỢỠĂƯỨỪỬỰỮÚÙỦỤŨÂÊẾỀỂỆỄÉÈẺẸẼÝỲỶỴỸÍÌỈỊĨ]|^\d+$/;
      ...
      result.buyer.taxCode = this.getBehind(tmpLine.strResult, ":")
        ...
        .trim();
      nextPos =
        this.getUntil(pageLines, tmpLine.nextPos, "(Amount)").nextPos + 1;
      let line = "";
