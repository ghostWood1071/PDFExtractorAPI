...
export class FastInvoiceExtractor extends PdfExtractor {
  ...
  private extractVAT(pageLines: string[], pageResult: PageContent) {
    ...
    pageResult.no = this.getBehind(tmpL.strResult, ":")
      .replace(/\#/g, "")
      .trim();
    ...
    pageResult.seller.taxCode = this.getBehind(tmpL.strResult, ":")
      .replace(/\#/g, "")
      .replace(/\s/g, "");

    nextPos =
      this.getUntil(pageLines, tmpL.nextPos, "Họ tên người mua hàng").nextPos +
      1;
    if (pageLines[nextPos].includes("Tên đơn vị")) {
      pageResult.buyer.companyName = this.getBehind(pageLines[nextPos], ":")
        .replace(/\#/g, "")
        .trim();
      nextPos = this.getUntil(pageLines, nextPos, "Mã số thuế").nextPos;
      tmpL = this.getUntil(pageLines, nextPos, "Đồng tiền thanh toán");
      pageResult.buyer.taxCode = this.getBehind(tmpL.strResult, ":")
        .replace(/\#/g, "")
        .replace(/\s/g, "");
    }
    ...
    if (pageResult.buyer.companyName == "")
      pageResult.buyer.companyName = this.getBehind(tmpL.strResult, ":")
        .replace(/\#/g, "")
        .replace("Mã số thuế", "")
        .trim();

    nextPos =
      this.getUntil(pageLines, tmpL.nextPos, "Số tài khoản").nextPos + 1;
    tmpL = this.getByFilter(pageLines, nextPos, (x) =>
      x.toLowerCase().includes("stt##tên hàng hóa")
    ); //this.getUntil(pageLines, nextPos, "Stt##Tên hàng hóa");
    if (
      pageResult.buyer.taxCode == "" ||
      !/^\d+$/.test(pageResult.buyer.taxCode)
    ) {
      pageResult.buyer.taxCode = tmpL.strResult
        .split(":")[2]
        .trim()
        .replace(/\s/g, "");
    }
  ...
  private extractExportTicket(pageLines: string[], pageResult: PageContent) {
    ...
    pageResult.buyer.companyName = this.getBehind(tmpL.strResult, ":").replace(
      /\#/g,
      ""
    );
    pageResult.serial = this.getBehind(pageLines[tmpL.nextPos], ":")
      .replace(/\#/, "")
      .trim();
  ...
  protected override processPage(pageLines: string[]) {
    ...
    else tmpL = this.extractVAT(pageLines, pageResult);
    ...
    if (exchangeRegex.test(pageLines[nextPos])) {
      pageResult.exchange_rate = parseFloat(
        pageLines[nextPos]
          .replace(/\s/g, "")
          .replace(/\#\#/g, "#")
          .split("#")[2]
