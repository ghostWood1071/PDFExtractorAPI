...
export class ExtractorService {
  ...
  private createExtractor(className: any, args: any) {
    ...
    if (className != null || className != undefined) return new className(args);
  ...
  public async getExtractor(fileContent: any) {
    if (this.supplier == "M-Invoice") {
      ...
      if (info.Creator.includes("Windows NT 6.1")) {
        return this.createExtractor(
          this.extractorDict[this.supplier][0],
          fileContent
        );
      } else {
      ...
      if (Array.isArray(extractorName)) {
        ...
        return this.createExtractor(
          this.currentExtractor[this.extractorIndex],
          fileContent
        );
      } else {
  ...
  public next(fileContent: any) {
    if (this.currentExtractor.length == 0) return null;
    this.extractorIndex++;
    return this.createExtractor(
      this.currentExtractor[this.extractorIndex],
      fileContent
