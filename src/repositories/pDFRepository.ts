...
export class PdfRepository {
  ...
  private getSupplier(info:any){
    for(let supplier of pdfSupplier){
      ...
      if(regex.test(info.Producer) || info.Producer == supplier.Producer || info.Producer.includes(supplier.Producer)){
  ...
  private async getResult(processor: any, fileName:string) {
    ...
    data.path = fileName;
    data.file_name = fileName.substring(fileName.lastIndexOf("/")+1, fileName.length);
    return data;
  }
  ...
  async process(listFileName:string[]):Promise<any> {
    ...
    for(let fileName of listFileName){
        ...
        if (supplier == null){
          throw new Error(`Định dạng file: ${fileName.substring(fileName.lastIndexOf("/")+1, fileName.length)} không được hỗ trợ`);
        }
        else{
  ...
  async checkVATExists(vat_invoice_ids: any){
    try {
      ...
      const [results] = await this.db.query(sql, [JSON.stringify(vat_invoice_ids)]);
      return results;
    } catch (error:any) {
